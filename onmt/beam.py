...
class Beam():
  ...
  def advance(self, word_prob):
    ...
    # 这个目的又是什么？topk_beam_index不是一定为0吗？topk_ids不是不变吗？ 
    topk_beam_index = topk_ids // num_words
    topk_ids %= num_words 
    # topk_beam_index: (size * 2,)
    # topk_ids: (size * 2,)
    # 第一返回的topk_seq是 tensor[[2],[2],[2],[2],[2]...], 长度为2 * n_bm
